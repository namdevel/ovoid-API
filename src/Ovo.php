...
class Ovo {
    ...
    */
    public function login2FA($phoneNumber) {
        $field = ["msisdn" => $phoneNumber, "device_id" => self::device_id, ];
    ...
    */
    public function login2FAverify($reff_id, $otpCode, $phoneNumber) {
        $field = ["msisdn" => $phoneNumber, "device_id" => self::device_id, "otp_code" => $otpCode, "reff_id" => $reff_id, ];
    ...
    */
    public function loginSecurityCode($securityCode, $phoneNumber, $otp_token) {
        $field = ["security_code" => $securityCode, "msisdn" => $phoneNumber, "device_id" => self::device_id, "otp_token" => $otp_token, ];
    ...
    */
    public function transactionHistory($page = 1, $limit = 10) {
        return self::Request(self::agw_api . "/payment/orders/v1/list?limit={$limit}&page={$page}", false, self::headers());
    }
    ...
    */
    public function isOVO($phoneNumber) {
        $field = ["mobile" => $phoneNumber, "amount" => 10000, ];
    ...
    */
    private function generateTrxId($amount) {
        $field = ["amount" => $amount, "actionMark" => self::action_mark, ];
    ...
    */
    private function unlockTrxId($amount, $trxId, $securityCode) {
        $field = ["trxId" => $trxId, "signature" => self::generateSignature($amount, $trxId), "securityCode" => $securityCode, ];
    ...
    */
    public function tfOVO($amount, $phoneNumber, $securityCode, $message = "") {
        ...
        if (self::getRes($verify)->fullName) {
            ...
            $field = ["amount" => $amount, "trxId" => $trxId, "to" => $phoneNumber, "message" => $message, ];
    ...
    */
    private function tfBankPrepare($bankCode, $bankNumber, $amount, $messages = "") {
        $field = ["accountNo" => $bankNumber, "bankCode" => $bankCode, "messages" => $messages, "amount" => $amount, ];
    ...
    */
    private function tfBankExecute($amount, $bankName, $bankCode, $bankAccountNumber, $bankAccountName, $trxId, $notes = "") {
        $field = ["bankName" => $bankName, "notes" => $notes, "transactionId" => $trxId, "accountNo" => self::getAccountNumber(), "accountName" => $bankAccountName, "accountNoDestination" => $bankAccountNumber, "bankCode" => $bankCode, "amount" => $amount, ];
    ...
    */
    private function headers() {
        $headers = ["content-type: application/json", "app-id: " . self::app_id, "app-version: " . self::app_version, "os: " . self::os_name, "user-agent: " . self::user_agent, 'client-id: ' . self::client_id, ];
        return $headers;
    }
